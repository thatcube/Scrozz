...
impl Frame {
    ...
    pub fn is_well_formed(&self) -> bool {
        let Some(min_stride) = usize::try_from(self.width())
            .ok()
            .and_then(|width| width.checked_mul(self.format.bytes_per_pixel()))
        else {
            return false;
        };
        let Some(required) = usize::try_from(self.height())
            .ok()
            .and_then(|height| self.stride.checked_mul(height))
        else {
            return false;
        };
...
mod tests {
    ...
    #[test]
    fn hostile_stride_overflow_is_not_well_formed() {
        let frame = Frame {
            data: Vec::new(),
