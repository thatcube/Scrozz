...
pub enum WindowSelection {
    ...
    PortalPicker {
        /// The portal interface that performs the selection.
        portal: String,
    },
...
impl WindowSelection {
    ...
    /// Whether the platform can select a window at all.
    #[must_use]
...
pub enum ShadowSupport {
    /// The compositor honours both answers.
    Toggleable,
    /// The source neither controls nor reports whether a shadow is present.
    Unchecked {
        /// Why no reliable shadow state is available.
        why: String,
    },
    /// Every capture arrives with the shadow.
    AlwaysIncluded {
        /// Why the shadow cannot be dropped.
        why: String,
    },
    /// No capture contains a shadow.
    AlwaysExcluded {
        /// Why the shadow is absent.
        why: String,
    },
}
...
impl ShadowSupport {
    ...
    /// What the shadow flag resolves to given what the user requested.
    #[must_use]
    pub const fn resolve(&self, requested: bool) -> bool {
        match self {
            Self::Toggleable => requested,
            Self::Unchecked { .. } | Self::AlwaysExcluded { .. } => false,
            Self::AlwaysIncluded { .. } => true,
        }
    }

    /// Why the control is fixed, if it is fixed.
    #[must_use]
...
pub struct WindowPickingCapability {
    ...
    /// Whether the shadow toggle is real.
    pub shadow: ShadowSupport,
    /// Whether captured pixels carry native transparency.
...
pub struct SourceApp {
    ...
    /// A stable per-platform application identifier.
...
impl SourceApp {
    /// Whether the OS supplied any application metadata.
    #[must_use]
    pub fn is_known(&self) -> bool {
        self.name.is_some() || self.identifier.is_some() || self.window_title.is_some()
    }

    /// The best available label for a compact badge.
    #[must_use]
