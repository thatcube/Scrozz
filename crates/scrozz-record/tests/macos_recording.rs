...
use scrozz_record::{
    OverlayLayer, OverlaySource, RecordingRequest, RecordingState, Resolution, VideoCodec,
};
...
fn opt_in_smoke_records_all_displays_and_a_region_across_their_seam() {
    ...
    let mut request = RecordingRequest::new(CaptureTarget::AllDisplays);
    request.destination = Some(output.0.clone());
    request.resolution = Resolution::LongestEdge(1_280);
    request.microphone = false;
    let session = attempt!(
    ...
    assert!(recording.frames > 0);
    assert!(recording.partial.is_none(), "{:?}", recording.partial);
    assert!(recording.size.width <= 1_280.0 && recording.size.height <= 1_280.0);
    ...
    assert!(recording.partial.is_none(), "{:?}", recording.partial);
    assert_eq!(
        recording.size.width as u32,
        (region.size.width * expected_scale) as u32 & !1
    );
    assert_eq!(
        recording.size.height as u32,
