...
fn encoder_plan_caps_height_preserves_aspect_and_rounds_even() {
    assert_eq!(output_dimensions(3840, 2160, Some(1080)), (1920, 1080));
    assert_eq!(output_dimensions(1512, 945, Some(720)), (1152, 720));
    assert_eq!(output_dimensions(1280, 720, Some(1080)), (1280, 720));
    assert_eq!(output_dimensions(101, 99, None), (100, 98));
}

#[test]
fn quality_and_frame_rate_change_the_encoder_plan() {
    let low = build(1920, 1080, 30, Quality::Low, None).unwrap();
    let high = build(1920, 1080, 30, Quality::High, None).unwrap();
    let sixty = build(1920, 1080, 60, Quality::Balanced, None).unwrap();
    let thirty = build(1920, 1080, 30, Quality::Balanced, None).unwrap();

    assert!(high.bitrate > low.bitrate * 3);
    assert_eq!(sixty.bitrate, thirty.bitrate * 2);
    assert_eq!(sixty.gop, 120);
    assert_eq!(
        build(1920, 1080, 0, Quality::Balanced, None),
