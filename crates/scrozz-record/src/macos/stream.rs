...
impl RecordingSession for MacRecordingSession {
    ...
    fn stop(mut self: Box<Self>) -> Result<Recording> {
        ...
        match (interactions, raw_summary) {
            (Some(mut interactions), Ok(Some(raw))) => match lock(&self.shared.raw_source).take() {
                Some(source) if raw.path == source.path() => {
                    interactions.attach_source(source);
                    recording = recording.with_interactions(interactions);
                }
                Some(source) => {
                    tracing::warn!(
                        expected = %raw.path.display(),
                        actual = %source.path().display(),
                        "private interaction source path changed; the final recording is intact"
                    );
                    recording.metadata.interaction_editable = Some(false);
                }
                None => {
                    tracing::warn!(
                        "private interaction source ownership was lost; the final recording is intact"
                    );
                    recording.metadata.interaction_editable = Some(false);
                }
            },
