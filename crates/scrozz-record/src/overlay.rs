...
mod tests {
    ...
    #[test]
    fn camera_only_presenter_drag_preserves_the_saved_pip_anchor() {
        let settings = CameraSettings {
            enabled: true,
            presenter: true,
            presenter_screen: false,
            position: OverlayAnchor::BottomRight,
            placement: Some(crate::settings::CameraPlacement::new(0.7, 0.4).unwrap()),
            ..CameraSettings::default()
        };

        let moved = move_camera(
            output(),
            16.0 / 9.0,
            24.0,
            30.0,
        ...
        assert_eq!(
            moved, settings,
            "a full-frame camera has nowhere to move and must retain its PiP preference"
        );
    }
