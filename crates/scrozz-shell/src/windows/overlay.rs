...
use windows::Win32::{
    ...
    UI::{
        ...
        WindowsAndMessaging::{
            ...
            WM_NCDESTROY, WS_EX_LAYERED, WS_EX_NOACTIVATE, WS_EX_TOOLWINDOW, WS_EX_TRANSPARENT,
        },
    },
};
...
impl WindowsOverlay {
    ...
    pub fn apply(&mut self, behavior: &OverlayBehavior) -> Result<OverlayReport> {
        ...
        Ok(OverlayReport {
            non_activating: true,
            detail: "Win32 WS_EX_NOACTIVATE + WM_MOUSEACTIVATE guard".into(),
        })
    }
    ...
    fn validate(&self) -> Result<()> {
        if !unsafe { IsWindow(Some(self.hwnd)) }.as_bool()
            ...
            || window_title(self.hwnd) != self.title
        {
