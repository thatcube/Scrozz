...
#[test]
fn opening_replays_a_marked_sidecar_share_update_even_when_capture_counts_match() {
    ...
    let pending = dir.path().join("pending-index").join("share-crash.pending");
    fs::write(&pending, format!("{}\n", id.0)).expect("write interrupted index marker");
    drop(store);
