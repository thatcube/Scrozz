...
const RECORD_COLUMNS: &str = "id, created_at, media_kind, pinned, app_name, app_identifier, \
     window_title, window_shadow, provenance, target_json, frame_json, image_hash, image_bytes, \
...
fn upsert_record(conn: &Connection, record: &StoredRecord) -> Result<()> {
    ...
    conn.execute(
        "INSERT INTO captures (
             id, created_at, stored_at, media_kind, pinned, app_name, app_identifier, window_title,
...
fn row_to_record(row: &Row<'_>) -> Result<CaptureRecord> {
    ...
    let image_hash: Option<String> = get(row, 11)?;
    let image_bytes: i64 = get(row, 12)?;
    let image_evicted_at: Option<i64> = get(row, 13)?;
    let ocr_text: Option<String> = get(row, 14)?;
    let annotation_count: i64 = get(row, 15)?;
