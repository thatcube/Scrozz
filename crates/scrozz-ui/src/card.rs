...
fn accessible_name(content: &CardContent<'_>) -> String {
    format!(
        "Capture {}, {}. Enter annotates; C copies; S saves; P pins; Delete closes. \
         Drag right or up to share.",
        content.name,
        content.dimensions()
    )
}
...
pub fn draw_card(
    ...
    .into_iter()
    .any(|action| focused == Some(control_id(frame.id, action)));
    let keyboard_reveal = if body.has_focus() || chrome_focused {
        1.0
    } else {
        0.0
    };
    let reveal = frame.reveal.clamp(0.0, 1.0).max(keyboard_reveal) * flat;
    ...
    if action.is_none() && body.has_focus() {
        action = ui.input(|input| {
            if input.key_pressed(egui::Key::Enter) || input.key_pressed(egui::Key::Space) {
                ...
                Some(CardAction::Copy)
            } else if input.key_pressed(egui::Key::S) {
                Some(CardAction::Save)
...
mod tests {
    ...
    fn the_card_body_accessible_name_identifies_content_and_drag_action() {
        ...
        assert!(label.contains("Enter annotates"), "{label}");
