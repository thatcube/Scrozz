...
fn preview_output_size(state: &EditorState) -> LogicalSize {
    if state.crop_mode() {
        ...
        document.set_orientation(state.display_orientation());
...
fn gestures(ui: &Ui, state: &mut EditorState, response: &egui::Response, view: &CanvasView) {
    ...
    let point = state.display_to_source(to_document(screen, view.subject, view.content));
    ...
    if response.drag_started() {
        ...
        let press = state.display_to_source(to_document(press_screen, view.subject, view.content));
        if pan_gesture {
...
fn draw_crop_scrim(
    ...
    for segment in state.active_crop_snap_segments() {
        ...
        painter.line_segment(
            [
                super::to_screen(state.source_to_display(from), view.subject, view.content),
                super::to_screen(state.source_to_display(to), view.subject, view.content),
            ],
...
fn cursor(ui: &Ui, state: &EditorState, response: &egui::Response, view: &CanvasView) {
    ...
    let crop_handle = state.active_crop_handle().or_else(|| {
        response.hover_pos().and_then(|screen| {
            let display = to_document(screen, view.subject, view.content);
