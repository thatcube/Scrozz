...
impl<'a> RecordingSettingsPanel<'a> {
    ...
    /// Marks a recording as running, which locks the preferences.
    ///
    /// A running recording is the one moment these values must not move: the
    /// engine has already installed whatever hooks the settings asked for, so
    /// the panel reports what is live instead of offering edits that the state
    ...
    pub fn show(mut self, ui: &mut Ui) -> RecordingSettingsResponse {
        ...
        let inner = panel(ui, self.theme, 500.0, |ui| {
            ...
            ui.add_enabled_ui(cursor && self.capabilities.cursor_smoothing, |ui| {
                ui.checkbox(
            ...
            if cursor && !self.capabilities.cursor_smoothing {
                caption(
                    ui,
                    self.theme,
                    "Pointer smoothing is unavailable in this recording engine.",
                );
            }
    ...
    /// Draws the read-only surface shown while a recording is running.
    ///
    /// It names exactly what is being observed, because the honest answer to
