...
/// The text-recognition pane of the ordinary Settings window.
///
/// A pane body, not a window: it draws into the `Ui` the Settings viewport
/// hands it, exactly like every other pane, so OCR never opens a second
/// settings surface of its own.
#[derive(Debug, Clone, Copy, Default)]
pub struct OcrSettings;

impl OcrSettings {
    /// Draws the pane body with the Settings window's own theme.
    #[must_use]
    pub fn body(&mut self, ui: &mut Ui, theme: &Theme) -> OcrSettingsResponse {
        let palette = theme.palette;
        ...
        ui.label(
            RichText::new(
                "Recognition and barcode decoding run on this machine. Nothing a \
                 capture contains is uploaded, and a decoded link is never opened \
                 for you.",
            )
            .font(theme.font(Text::Body))
            .color(palette.text_muted),
        ...
        response.show_onboarding = ui
            .add(
                Button::new(
                    RichText::new("Show OCR introduction again").font(theme.font(Text::Button)),
                )
                .corner_radius(corner(Radius::BUTTON))
    ...
    /// Draws the pane standalone, resolving the theme from `ui`.
    #[must_use]
