...
pub struct HistoryEntry {
    ...
    /// Why this build cannot decode or render the stored document.
...
pub enum HistoryAction {
    ...
    /// Pre-render a selected capture before the user begins dragging.
    PrepareDrag(CaptureId),
...
pub struct HistoryViewModel {
    ...
    drag_loading: Option<CaptureId>,
    drag_ready: Option<CaptureId>,
    request: u64,
...
impl HistoryViewModel {
    ...
    pub fn select(&mut self, id: &CaptureId) {
        if self.entries.iter().any(|entry| entry.id == *id) {
            ...
            if can_drag && self.drag_ready.as_ref() != Some(id) {
                ...
                self.actions
                    .push_back(HistoryAction::PrepareDrag(id.clone()));
            }
        }
    }
    ...
    pub fn close(&mut self) {
        ...
        self.delete_armed = None;
        self.drag_loading = None;
        self.drag_ready = None;
    }

    /// Takes the one-shot request to focus a newly opened or reused viewport.
    #[must_use]
    ...
    pub fn deleted(&mut self, id: &CaptureId) {
        ...
        self.selected = None;
        self.delete_armed = None;
        if self.drag_loading.as_ref() == Some(id) {
            self.drag_loading = None;
        }
        if self.drag_ready.as_ref() == Some(id) {
            self.drag_ready = None;
        }
        self.actions.retain(
            |action| !matches!(action, HistoryAction::PrepareDrag(capture) if capture == id),
        );
    ...
    /// Marks a selected capture ready for immediate native drag startup.
    pub fn drag_prepared(&mut self, id: &CaptureId) {
        if self.selected.as_ref() == Some(id) {
            self.drag_loading = None;
            self.drag_ready = Some(id.clone());
        }
    }

    /// Clears an unsuccessful preload without hiding the rest of history.
    pub fn drag_preparation_failed(&mut self, id: &CaptureId, error: impl Into<String>) {
        if self.drag_loading.as_ref() == Some(id) {
            self.drag_loading = None;
        }
        self.notice = Some(error.into());
    ...
    /// Invalidates pre-rendered bytes after the editable document changes.
    pub fn invalidate_drag(&mut self, id: &CaptureId) {
        if self.drag_ready.as_ref() == Some(id) {
            self.drag_ready = None;
        }
        if self.drag_loading.as_ref() == Some(id) {
    ...
    /// Whether the host must wake root logic to dispatch a queued action.
    #[must_use]
        ...
        let Some(thumbnail) = &entry.thumbnail else {
            ui.painter().text(
                ...
                if entry.content_error.is_some() {
                    "Preview unavailable\nThis build cannot open the saved edits"
                } else if entry.image_present {
                    "Preview unavailable"
                } else {
    ...
    fn detail(&mut self, ui: &mut Ui, theme: &Theme, entry: &HistoryEntry) {
        ...
        ui.add_space(Space::SM);
        let drag_ready = self.drag_ready.as_ref() == Some(&entry.id);
        let sense = if drag_ready {
            Sense::click_and_drag()
        } else {
            Sense::hover()
        };
        let (preview, drag) = ui.allocate_exact_size(vec2(ui.available_width(), 208.0), sense);
        ...
        if let Some(error) = &entry.content_error {
            ui.add_space(Space::SM);
            ui.label(
                RichText::new(error)
                    .font(theme.font(Text::Caption))
                    .color(Color32::from_rgb(0xD9, 0x4B, 0x4B)),
            );
        }
...
/// The ordinary desktop window used for capture history.
#[must_use]
pub fn viewport_builder(active: bool) -> ViewportBuilder {
...
mod tests {
    ...
    #[test]
    fn selecting_prepares_drag_bytes_before_the_preview_can_be_dragged() {
        ...
        assert!(matches!(
            model.drain_actions().as_slice(),
            [HistoryAction::PrepareDrag(preparing)] if preparing == &id
        ));
        assert_eq!(model.drag_loading.as_ref(), Some(&id));
    ...
    #[test]
    fn first_open_and_reopen_request_focus_without_repainting_it_forever() {
        let now = Timestamp(1_000);
        let mut model = HistoryViewModel::new(now);

        model.open(now);
        assert!(model.take_focus_request());
        assert!(!model.take_focus_request());

        model.open(Timestamp(2_000));
        assert!(model.take_focus_request());
        model.close();
        assert!(!model.take_focus_request());
    }
