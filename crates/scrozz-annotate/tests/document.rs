...
#[test]
fn restoring_undo_state_invalidates_existing_analysis() {
    let mut doc = document(200, 200);
    doc.add_default(Annotation::Rectangle(rect(1.0, 2.0, 3.0, 4.0)));
    let restored = doc.data();
    doc.add_default(Annotation::Highlight(rect(20.0, 20.0, 30.0, 10.0)));
    let scanned = doc.revision();

    doc.restore(restored).expect("restore");

    assert!(doc.revision() > scanned);
    assert_eq!(doc.len(), 1);
}

#[test]
fn failed_source_replacement_is_atomic_and_keeps_its_revision() {
    let mut doc = document(1_000, 1_000);
    doc.set_crop(Some(rect(500.0, 500.0, 100.0, 100.0)))
        .expect("crop");
    let revision = doc.revision();
    let crop = doc.crop();

    let error = doc
        .replace_source(region_capture(400, 400))
        .expect_err("the crop no longer overlaps");

    assert!(error.to_string().contains("does not overlap"), "{error}");
    assert_eq!(doc.source().frame.width(), 1_000);
    assert_eq!(doc.crop(), crop);
