...
use scrozz_core::{CaptureBackend, CaptureRequest, CaptureTarget, Error};
...
fn run() -> Result<(), Outcome> {
    ...
    match backend.capture(&request) {
        Ok(second) => {
            report(&second)?;
            eprintln!(
                "wayland-smoke: second capture succeeded. If no dialog appeared, restore \
                 tokens are working."
            );
        }
        Err(Error::Cancelled) => {
...
fn report(capture: &scrozz_core::Capture) -> Result<(), Outcome> {
