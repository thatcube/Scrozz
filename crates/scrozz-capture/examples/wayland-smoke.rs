...
fn main() -> ExitCode {
    ...
    let stale_token = env::args().any(|argument| argument == "--stale-token");
...
fn run(stale_token: bool) -> Result<(), Outcome> {
...
fn seed_stale_token(display_id: &str) -> Result<(), Outcome> {
    use scrozz_capture::linux::wayland::restore::{TokenKey, TokenStore, token_path};
    ...
    .ok_or_else(|| {
        Outcome::Failed("could not resolve the isolated portal-token path".to_owned())
    })?;
    let mut tokens = TokenStore::new();
    tokens.set(
        TokenKey::for_display(display_id),
        "scrozz-intentionally-stale-restore-token",
    );
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).map_err(|error| {
            Outcome::Failed(format!(
                "could not create isolated token directory {}: {error}",
                parent.display()
            ))
        })?;
    }
    std::fs::write(&path, tokens.serialise()).map_err(|error| {
        Outcome::Failed(format!(
            "could not plant the stale token at {}: {error}",
            path.display()
        ))
    })?;
    eprintln!("wayland-smoke: planted an intentionally stale token for display {display_id}");
    Ok(())
}
