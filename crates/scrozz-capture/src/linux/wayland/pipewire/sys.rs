...
symbols! {
    ...
    pw_stream_flush:
        unsafe extern "C" fn(*mut c_void, bool) -> c_int = b"pw_stream_flush\0",
        ...
        let why = match self {
            Self::Missing(detail) => format!(
                "PipeWire is not installed, so the frames the desktop portal offers cannot be \
                 ...
                 install pipewire libpipewire-0.3-0t64` on Ubuntu 24.04+, \
                 `sudo apt install pipewire libpipewire-0.3-0` on older Debian/Ubuntu, `sudo dnf install \
