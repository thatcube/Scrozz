...
pub enum CropError {
    ...
    Outside,
    /// The region overlaps this stream but also needs pixels from another one.
    SpansDisplays,
}
...
impl CropError {
    ...
    pub fn into_error(self, compositor: &str) -> scrozz_core::Error {
        ...
        match self {
            ...
            Self::SpansDisplays => scrozz_core::Error::InvalidRequest(
                "the selected region crosses a display boundary, but the Wayland portal granted \
                 only one monitor stream. Select a region wholly inside one display"
                    .into(),
            ),
        }
    }
...
pub fn plan_crop(
    ...
    if right <= 0.0
        || bottom <= 0.0
        || left >= f64::from(frame_size.0)
        || top >= f64::from(frame_size.1)
    {
        return Err(CropError::Outside);
    }
    if left < 0.0
        || top < 0.0
        || right > f64::from(frame_size.0)
        || bottom > f64::from(frame_size.1)
    {
        return Err(CropError::SpansDisplays);
    }
