...
use scrozz_core::{Capture, CaptureBackend, CaptureRequest, Error, Frame, Result, ScrollDriver};
...
pub fn capture_with_cancellation(
    ...
    {
        cancellation.check()?;
        macos::ScreenCaptureKitBackend::with_cancellation(cancellation).capture(request)
    }
...
/// # Errors
///
/// Returns [`Error::Platform`] when the selected native input service cannot be
/// reached, or [`Error::Unsupported`] on an unknown operating system.
