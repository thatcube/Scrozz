...
fn align_axis_search(
    ...
    loop {
        let (best, runner_up, confidence) = best_verified(&shortlist, config.basin_radius)?;
...
fn best_verified(
    ...
    let runner_up = basins
        ...
        // Equal-score alternatives remain ambiguous even when they form one
        // flat local plateau. Otherwise, nearby verified minima are one basin.
        .filter(|candidate| {
            candidate.raw_score == best.raw_score
                || candidate.delta.abs_diff(best.delta) > basin_radius
        })
...
fn profile_sad(profiles: &CoarseProfiles, span: AnalysisSpan, delta: u32) -> (u32, u64) {
    ...
    let samples = overlap.clamp(1, MAX_COARSE_AXIS_SAMPLES);
    ...
    // Ranking stays capped at a small sampled sweep, but certification must
    // account for every overlapping profile. A sampled sum is a valid lower
    // bound, yet becomes too weak to eliminate clearly inferior offsets on
    // large viewports when compared with the full overlap's pixel count.
    let pixel_sad_lower_bound = (0..overlap).fold(0u64, |sum, position| {
        let previous_position = span.start + delta + position;
        let current_position = span.start + position;
        sum.saturating_add(profiles.pixel_sad_lower_bound(previous_position, current_position))
    });
    (
...
mod tests {
    ...
    fn verified_candidate(delta: u32, raw_score: u32) -> Candidate {
        Candidate {
            delta,
            raw_score,
            adjusted_score: raw_score,
            pixel_sad_lower_bound: 0,
            overlap: 100 - delta,
            perpendicular: AnalysisSpan::full(20),
        }
    }

    #[test]
    fn confidence_uses_the_next_distinct_verified_basin() {
        let candidates = [
            verified_candidate(10, 1),
            verified_candidate(11, 2),
            verified_candidate(30, 6),
        ];

        let (best, runner_up, confidence) =
            best_verified(&candidates, 3).expect("verified candidates");

        assert_eq!(best.delta, 10);
        assert_eq!(runner_up.map(|candidate| candidate.delta), Some(30));
        assert_eq!(confidence, 5);
    }

    #[test]
    fn a_flat_verified_plateau_remains_ambiguous_inside_one_basin() {
        let candidates = [
            verified_candidate(10, 1),
            verified_candidate(11, 1),
            verified_candidate(30, 8),
        ];
        ...
        assert_eq!(best.delta, 10);
        assert_eq!(runner_up.map(|candidate| candidate.delta), Some(11));
