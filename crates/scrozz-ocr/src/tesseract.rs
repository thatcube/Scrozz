...
pub(crate) fn recognize(frame: &Frame, options: &Options) -> Result<Vec<TextBlock>> {
    if options.automatic_language_detection {
        return Err(Error::Unsupported {
            what: "automatic OCR language detection".to_owned(),
            why: "the portable Tesseract backend requires an explicit or system language; it \
                  cannot infer language from arbitrary image content"
                .to_owned(),
        });
    }
    ...
    let installed_languages = list_languages(&installation)?;
...
/// Lists traineddata models available in the artifact-local payload.
pub(crate) fn available_languages() -> Result<Vec<String>> {
    list_languages(&tesseract_installation()?)
}
...
fn recognition_arguments(selected_languages: &[String], options: &Options) -> Vec<OsString> {
    ...
    // A bare `tsv` config adds an undeclared dependency on
    // tessdata/configs/tsv. This flag works with the documented payload alone.
    arguments.extend([
        OsString::from("-c"),
...
fn list_languages(installation: &TesseractInstallation) -> Result<Vec<String>> {
    let output = run_tesseract(
        installation,
...
fn validate_tesseract_directory(
    ...
    if !program.is_file() || !tessdata.is_dir() || !english_model.is_file() {
        let source = if explicitly_configured {
            ...
            "the portable artifact must contain".to_owned()
        };
        return Err(Error::Unsupported {
            what: if explicitly_configured {
                "text recognition with the configured Tesseract installation".to_owned()
            } else {
                "text recognition from the portable Windows artifact".to_owned()
            },
            why: format!(
...
fn join_writer(writer: Option<thread::JoinHandle<std::io::Result<()>>>) -> Result<()> {
    ...
    writer
        .join()
        .map_err(|_| Error::Platform("the Tesseract stdin worker panicked".to_owned()))?
        .map_err(|error| {
            Error::Io(IoError::new(
                error.kind(),
...
fn resolve_languages(requested: &[String], installed: &[String]) -> Result<Vec<String>> {
    ...
    for requested_tag in requested {
        ...
        {
            selected.push(installed_tag.to_owned());
        }
    }
...
fn resolve_system_languages(installed: &[String]) -> Result<Vec<String>> {
    ...
    if requested.is_empty() {
        return Err(Error::Unsupported {
            what: "text recognition in the system language".to_owned(),
            why: format!(
...
fn tesseract_language_aliases(tag: &str) -> Vec<String> {
    ...
    if matches!(base, "zh" | "zho" | "chi") {
        ...
        return vec![if traditional { "chi_tra" } else { "chi_sim" }.to_owned()];
    }

    if matches!(base, "nb" | "nob" | "nn" | "nno") {
        return vec!["nor".to_owned()];
    }
    ...
    script_model.map_or_else(
        || vec![model.to_owned()],
        |script_model| vec![script_model.to_owned(), model.to_owned()],
    )
}
...
fn spawn_error(program: &OsStr, error: std::io::Error) -> Error {
    if error.kind() == ErrorKind::NotFound {
        Error::Unsupported {
            what: "text recognition with Tesseract".to_owned(),
            why: format!(
...
mod tests {
    ...
    #[test]
    fn recognition_always_passes_an_explicit_language_and_inline_tsv_flag() {
        let arguments = recognition_arguments(&["eng".to_owned()], &Options::default());
        ...
        assert!(!arguments.iter().any(|argument| argument == "tsv"));
    }

    #[test]
    fn resolves_bcp47_tags_to_installed_traineddata() {
        assert_eq!(
            resolve_languages(&["en-US".to_owned()], &["eng".to_owned()]).unwrap(),
            ["eng"]
        );
        assert_eq!(
            resolve_languages(&["ro-RO".to_owned()], &["ron".to_owned()]).unwrap(),
            ["ron"]
        );
        assert_eq!(
            resolve_languages(&["nn-NO".to_owned()], &["nor".to_owned()]).unwrap(),
            ["nor"]
        );
    }

    #[test]
    fn portable_auto_detection_is_rejected_honestly() {
        let error = recognize(
            &frame(1, 1),
            &Options::default().with_automatic_language_detection(true),
        )
        .unwrap_err();
        assert!(matches!(error, Error::Unsupported { what, .. } if what.contains("automatic")));
    }
