...
| Scrolling capture | 🟡 | 🟡 | 🟡 | Draw an exact area, choose Manual or Automatic, press Start, then scroll once in any direction. Scrozz detects up/down/left/right from the pixels; both modes wait for Finish before saving. Idle pauses, lost overlap, and safety limits never save automatically. macOS Auto requires the pointer inside the selected area and uses guarded native wheel input; Windows and X11 have native drivers awaiting real-desktop smoke, while Wayland is deliberately manual |
