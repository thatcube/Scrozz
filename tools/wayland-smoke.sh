...
if ! source_status=$(git status --porcelain --untracked-files=all); then
  echo "wayland-smoke: could not verify the source tree provenance with git status" >&2
  exit 1
fi
if [[ -n "$source_status" ]]; then
  echo "wayland-smoke: source tree has tracked or untracked inputs; commit or remove them before recording native evidence." >&2
  exit 1
fi
...
isolated_state=""
cleanup() {
  [[ -z "$trace_log" ]] || rm -f "$trace_log"
  [[ -z "$restore_log" ]] || rm -f "$restore_log"
  if [[ -n "$isolated_state" && -f "$isolated_state/.scrozz-smoke-owned" ]]; then
...
if [[ "$STALE_TOKEN" == "1" ]]; then
  isolated_state=$(mktemp -d "${TMPDIR:-/tmp}/scrozz-wayland-state.XXXXXX") || {
    echo "wayland-smoke: could not create an isolated state directory" >&2
    exit 1
  }
  if [[ "$isolated_state" != /* || ! -d "$isolated_state" ]] ||
     ! : >"$isolated_state/.scrozz-smoke-owned"; then
    echo "wayland-smoke: temporary state directory is not an owned absolute directory" >&2
    exit 1
  fi
