...
| QA-08 | Configurable auto-close behavior | T1 | **Implemented:** disabled by default; Hide refuses to destroy the only retained artifact, while Save then hide closes only after confirmed export |
...
shared native infrastructure used by selection, cards, and pins. Temporary
`OverlayApp`, `OverlayOptions`, `OverlayGeometry`, `OverlayEvent`, and
`OverlayHandle` public aliases remain as source-compatibility shims for
downstream code; new code uses the explicit names. Unknown settings and values
from newer document versions survive atomic read-modify-write round trips.
