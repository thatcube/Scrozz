...
- **Drag-out from the Recent Captures Overlay** — the hero interaction (see D12)
...
## D12 — The Recent Captures Overlay is the primary interface; drag-out is the hero action

**Decision.** The **Recent Captures Overlay** is a vertical list of captures, not a
...
- the **Recent Captures Overlay** (D28) cannot be anchored to the bottom-left;
...
revision to Export Location before releasing live pixels, and keeps those
pixels in memory if that recovery export fails.
...
remain because they cover shared native or selector infrastructure. The old
`OverlayApp`, `OverlayOptions`, `OverlayGeometry`, `OverlayEvent`, and
`OverlayHandle` names remain only as source-compatibility aliases while
