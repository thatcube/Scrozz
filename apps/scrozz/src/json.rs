...
//!
//! # Why this exists alongside `serde_json`
//!
//! Settings persistence uses `serde_json`, but command output stays explicit.
//! Per decision D11 the `--json` output is a **scripting contract**, so the
//! schema deserves to be a literal, greppable thing rather than whatever a
//! derive macro happened to emit from a struct that someone later reorders.
//!
