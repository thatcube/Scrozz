...
fn capture(args: &CaptureArgs) -> CliResult<Report> {
    ...
    let (capture, mut terminal_cancel) = match target {
        TargetSpec::Scrolling { axis, .. } => {
            let (capture, cancel) = scrolling_capture(backend.as_ref(), request, axis)?;
            (capture, Some(cancel))
        }
        _ => (backend.capture(&request)?, None),
    };
...
fn scrolling_capture(
    ...
    let capture = if std::io::stderr().is_terminal() {
        ...
        scrolling_capture_with(backend, request, axis, &mut cancel, |event| {
    ...
    F: FnMut(Progress),
{
    ...
    let source = CaptureFrameSource {
        session: scrozz_capture::frame_session(request.clone()).map_err(CliError::Core)?,
        crop,
    };
...
fn resolved_native_scrolling_target(
    ...
    request.include_window_shadow = false;
...
fn is_capture_control_window(window: &Window) -> bool {
    let Some(application) = window.application.as_deref() else {
        return false;
    };
...
mod tests {
    ...
    #[test]
    fn scrolling_uses_the_frontmost_application_window_on_the_selected_display() {
        ...
        assert_eq!(selected.id, WindowId("front".to_owned()));
    }
    ...
    fn output_publication_and_abort_have_one_ordered_winner() {
        ...
        let mut cancel = Some(TerminalCancellation { state });
        assert!(matches!(
