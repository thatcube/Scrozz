...
///
/// Publishing uses a same-directory hard link. That makes the final filename
/// appear atomically only after every byte is durable, and it never overwrites a
/// file another process created between staging and commit.
#[derive(Debug)]
    ...
    pub fn commit(mut self) -> CliResult<PathBuf> {
            ...
            && let Err(error) = fs::remove_file(&temporary)
        {
            tracing::warn!(
                ...
                %error,
                "published capture but could not remove its hidden staging link"
            );
        }
        Ok(destination)
    }
