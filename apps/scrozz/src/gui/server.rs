...
impl Server {
    ...
    pub fn poll(&mut self) -> Option<Request> {
        ...
        if let Some(admission) = self.admission.try_acquire() {
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn admission_is_held_until_completed_work_is_dropped() {
        let admission = Arc::new(Admission::default());
        let permits: Vec<_> = (0..MAX_PENDING_REQUESTS)
            .map(|_| admission.try_acquire().expect("permit within bound"))
            .collect();
        assert!(
            admission.try_acquire().is_none(),
            "pending, queued, and in-flight requests share one bound"
        );

        drop(permits);
        assert!(
            admission.try_acquire().is_some(),
            "completion releases admission"
        );
    }
