...
impl Worker {
    ...
    fn take(&mut self, kind: CaptureKind, card: CardId) -> CliResult<Card> {
        ...
        let request = CaptureRequest::new(target);
        let capture = backend.capture(&request)?;
        self.build_card(kind, card, capture)
    }
        ...
        let result = (|| -> CliResult<()> {
            ...
            match backend.window_selection() {
                ...
                WindowSelection::PortalPicker { .. } => {
                    ...
                    let built = self.build_card(CaptureKind::Window, card, capture)?;
                    if self.is_cancelled(card) {
                        return Ok(());
                    }
                    let _ = self.outcomes.send(Outcome::Ready(Box::new(built)));
    ...
    fn commit_window(&mut self, card: CardId, window: WindowId) {
        ...
        match result {
            Ok(capture) => {
                self.window_pickers.remove(&card);
                match self.build_card(CaptureKind::Window, card, capture) {
                    Ok(built) => {
                        if !self.is_cancelled(card) {
                            let _ = self.outcomes.send(Outcome::Ready(Box::new(built)));
                        }
    ...
    fn build_card(&mut self, kind: CaptureKind, card: CardId, capture: Capture) -> CliResult<Card> {
        if self.is_cancelled(card) {
...
mod tests {
    ...
    fn window_output_keeps_native_edges_stride_and_alpha_without_compositing() {
        ...
        let card = worker
            .build_card(CaptureKind::Window, CardId(9), capture)
    ...
    fn a_cancelled_window_card_is_not_encoded_or_cached() {
        ...
        assert!(matches!(
            worker.build_card(CaptureKind::Window, CardId(9), capture),
