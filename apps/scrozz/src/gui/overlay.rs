...
mod tests {
    ...
    #[test]
    fn accepted_drag_keeps_identity_until_the_dismissal_is_translated() {
        let mut surface = OverlayCards::new(OverlayHandle::new());
        let ours = CardId(5);
        let theirs = scrozz_ui::stack::CardId(9);
        surface.pending.push_back((ours, true));
        ...
        surface.finish_drag(ours, true);
        assert_eq!(surface.reverse.get(&ours), Some(&theirs.0));
        assert_eq!(surface.mapped.get(&theirs.0), Some(&ours));

        surface.translate(
            OverlayEvent::Dismissed {
                id: theirs,
                reason: scrozz_ui::DismissReason::DragOut,
            },
            &mut events,
        );

        assert_eq!(events, vec![CardEvent::Dismiss(ours)]);
        assert_eq!(surface.recent.back(), Some(&(ours, theirs.0)));
        assert!(!surface.reverse.contains_key(&ours));
    }
