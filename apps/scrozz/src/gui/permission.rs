...
impl PendingCapture {
    /// Builds a request from the app's stable origin names.
    #[must_use]
    pub fn new(kind: CaptureKind, origin: &'static str) -> Self {
        let invocation = match origin {
            "hotkey" => Invocation::Shortcut,
            "startup" => Invocation::Launch,
            _ => Invocation::Explicit,
        };
        Self {
            kind,
            origin,
