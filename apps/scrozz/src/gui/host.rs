...
use scrozz_core::{
    ...
    ScaleFactor, SelectionHost, SelectionOptions, SelectionOutcome,
};
use scrozz_ui::{
    OverlayHandle,
...
fn work_area() -> (OverlayGeometry, Option<DisplayId>, Option<ScaleFactor>) {
    #[cfg(target_os = "macos")]
    {
        match scrozz_shell::macos::display::active_display() {
            Ok(display) => {
                let geometry = geometry_for_display(&display);
                return (geometry, Some(display.id), Some(display.scale));
            }
            Err(err) => {
...
mod tests {
    ...
    fn first_card_waits_for_two_settled_native_measurements_at_every_scale() {
        for scale in [1.0, 1.25, 2.0] {
            ...
            .slot_rect(0);
            assert_eq!(slot.size(), egui::vec2(210.0, 150.0));
            assert_eq!(
                slot.size() * scale as f32,
                egui::vec2(210.0 * scale as f32, 150.0 * scale as f32)
            );
        }
    }
