...
impl Driver {
    ...
    fn show_settings(&mut self, ctx: &egui::Context) {
        ...
        } else if let Some(settings) = edits.recent_captures_overlay_preview {
            self.app.preview_recent_captures_overlay(settings);
        }
    }
}
...
impl eframe::App for Driver {
    ...
    fn logic(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        #[cfg(not(target_os = "macos"))]
        let disconnected_display_check_due = now >= self.next_active_display_refresh;
        #[cfg(target_os = "macos")]
